...
func NewTargetInfoGetter(tls *common.TLS, db *sql.DB, pdAddr, keyspaceName string) backend.TargetInfoGetter {
...
func NewBackend(
	...
	config.adjust()
	pdCtl, err := pdutil.NewPdController(ctx, keyspaceName, config.PDAddr, tls.TLSConfig(), tls.ToPDSecurityOption())
	if err != nil {
	...
	spkv, err := keyspace.NewEtcdSafePointKV(strings.Split(config.PDAddr, ","), pdCliForTiKV.GetCodec(), tls.TLSConfig())
	if err != nil {
