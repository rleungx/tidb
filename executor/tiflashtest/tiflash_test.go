...
func TestMPPRecovery(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/executor/mpp_recovery_test_mock_enable", "return()"))
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/executor/mpp_recovery_test_ignore_recovery_err", "return()"))
	// Test different chunk size. And force one mpp err.
	{
		require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/executor/mpp_recovery_test_max_err_times", "return(1)"))
		...
		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/executor/mpp_recovery_test_max_err_times"))
	}

	// Test exceeds max recovery times. Default max times is 3.
	{
		require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/executor/mpp_recovery_test_max_err_times", "return(5)"))
	...
	{
		require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/executor/mpp_recovery_test_max_err_times", "return(0)"))

		tk.MustExec("set @@tidb_max_chunk_size = 32")
		expectedHoldRows := 4 * 32
		require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/executor/mpp_recovery_test_hold_size", fmt.Sprintf("1*return(%d)", expectedHoldRows)))
		tk.MustQuery(sql).Check(testkit.Rows(checkStrs...))
		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/executor/mpp_recovery_test_hold_size"))

		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/executor/mpp_recovery_test_max_err_times"))
	}
	require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/executor/mpp_recovery_test_ignore_recovery_err"))
	require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/executor/mpp_recovery_test_mock_enable"))

	{
		// We have 2 mock tiflash, but the table is small, so only 1 tiflash node is in computation.
		require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/executor/mpp_recovery_test_check_node_cnt", "return(1)"))

		tk.MustExec("set @@tidb_max_chunk_size = 32")
		tk.MustQuery(sql).Check(testkit.Rows(checkStrs...))

		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/executor/mpp_recovery_test_check_node_cnt"))
	}
