...
func (s *mockGCSSuite) TestErrorMessage() {
	...
	checkClientErrorMessage(s.T(), err,
		"ERROR 8158 (HY000): The URI of INFILE is invalid. Reason: storage abc not support yet. Please provide a valid URI, such as")
	err = s.tk.ExecToErr("LOAD DATA INFILE 's3://no-network' INTO TABLE t;")
	checkClientErrorMessage(s.T(), err,
		"ERROR 8159 (HY000): Access to the source file has been denied.")
