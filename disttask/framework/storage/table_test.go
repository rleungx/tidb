...
func TestGlobalTaskTable(t *testing.T) {
	...
	cancelling, err := gm.IsGlobalTaskCancelling(ctx, task.ID)
	require.NoError(t, err)
	require.False(t, cancelling)

	require.NoError(t, gm.CancelGlobalTask(ctx, task.ID))
	cancelling, err = gm.IsGlobalTaskCancelling(ctx, task.ID)
	require.NoError(t, err)
...
func TestSubTaskTable(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 1, len(subtasks))
	require.NoError(t, sm.AddNewSubTask(ctx, 6, proto.StepInit, "tidb2", []byte("test"), proto.TaskTypeExample, false))
...
func TestTaskHistoryTable(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = gm.AddNewGlobalTask(ctx, "2", proto.TaskTypeExample, 1, nil)
	require.NoError(t, err)
