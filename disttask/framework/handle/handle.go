...
func SubmitGlobalTask(ctx context.Context, taskKey string, taskType proto.TaskType, concurrency int, taskMeta []byte) (*proto.Task, error) {
	...
	if globalTask == nil {
		...
		if intest.InTest {
			globalTask, err = globalTaskManager.GetNewGlobalTask(ctx)
		} else {
...
func waitTaskByKey(ctx context.Context, taskKey string, matchFn func(*proto.Task) bool) (*proto.Task, error) {
	globalTaskManager, err := storage.GetTaskManager()
	if err != nil {
		return nil, err
	}
	...
	logger := logutil.Logger(ctx).With(zap.String("task-key", taskKey))
	for {
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
		case <-ticker.C:
			found, err := globalTaskManager.GetGlobalTaskByKeyWithHistory(ctx, taskKey)
			if err != nil {
				logger.Error("cannot get global task during waiting", zap.Error(err))
				continue
			}
			if found == nil {
				return nil, errors.Errorf("cannot find global task with taskKey %s", taskKey)
			}
...
// WaitGlobalTaskByKey waits for a global task state changed.
func WaitGlobalTaskByKey(ctx context.Context, taskKey string) error {
	logger := logutil.Logger(ctx).With(zap.String("task-key", taskKey))
	found, err := waitTaskByKey(ctx, taskKey, func(t *proto.Task) bool {
		return t.IsDone() || t.State == proto.TaskStatePaused
	})
	if err != nil {
		return err
