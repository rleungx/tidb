...
// ConvertTaskIDInUT converts taskID to little endian.
func ConvertTaskIDInUT(taskID int64) int64 {
	bytes := make([]byte, 8)
	binary.LittleEndian.PutUint64(bytes, uint64(taskID))

	var res int64
	for i := 0; i < 4; i++ {
		res += int64(bytes[i]) << uint(i*8)
	}
	return res
}
