...
func matchServerInfo(serverInfos []*infosync.ServerInfo, id string, task *proto.Task) bool {
	...
	if variable.EnableDistTask.Load() && tidbworker.IsBgTaskMaster(string(task.Type)) {
		return tidbworker.IsWorkerExecID(id, tidbworker.TaskWorkerType(string(task.Type)))
	}
	return disttaskutil.MatchServerInfo(serverInfos, id)
}
