...
import (
	...
	"github.com/pingcap/tidb/disttask/framework/testutil"
...
func runOneTask(ctx context.Context, t *testing.T, mgr *storage.TaskManager, taskKey string, subtaskCnt int) {
	...
	task, err := mgr.GetNewGlobalTask(ctx)
	require.NoError(t, err)
	...
	task, err = mgr.GetGlobalTaskByID(ctx, task.ID)
	require.NoError(t, err)
	require.NotNil(t, task)
...
func TestSchedulerBasic(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/disttask/framework/scheduler/breakInSchedulerUT", "return()"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/domain/MockDisableDistTask"))
		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/disttask/framework/scheduler/breakInSchedulerUT"))
	}()
