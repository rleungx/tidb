...
const (
	...
	// Expect to use AES-256 for log encryption
	expectedEncryptKeyLength = 32
)
...
// InitWithConfig initialize a manager with configuration
func (m *LogManager) InitWithConfig(conf *config.AuditLog) (err error) {
	m.Lock()
	defer m.Unlock()
	...
	logFilepath, err := getFilePath(conf.Path, string(conf.Format))
	if err != nil {
		return err
	}

	var encryptKey, encryptIv string
	if conf.EncryptKey != "" {
		if len(conf.EncryptKey) != expectedEncryptKeyLength {
			return fmt.Errorf("bad encrypt key, %d != %d", len(conf.EncryptKey), expectedEncryptKeyLength)
		}
		encryptKey = conf.EncryptKey
		encryptIv = conf.EncryptKey[:aes.BlockSize]
	}
	...
	m.loggerCfg = LoggerConfig{
		...
		zapLoggerConfig: zapLoggerConfig{
			...
			FileMaxSize:         conf.MaxFilesize,
			FileMaxLifetime:     conf.MaxLifetime,
			FileReservedBackups: conf.ReservedBackups,
			FileReservedDays:    conf.ReservedDays,
			LogFormat:           string(conf.Format),
		},
		Redact:     conf.Redacted,
...
func (m *LogManager) bootstrapSQL() error {
	sqlList := []string{
		CreateFilterTableSQL,
		CreateFilterRuleTableSQL,
	}

	return m.runSQLInExecutor(func(exec sqlexec.SQLExecutor) error {
		for _, sql := range sqlList {
			if _, err := executeSQL(m.ctx, exec, sql); err != nil {
				return err
			}
		}
		return nil
