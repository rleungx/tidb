...
func TestAuditLogDefault(t *testing.T) {
	...
	require.Equal(t, "custom-tidb-audit.log", globalLogManager.GetLogConfigPath())
	require.Equal(t, filepath.Join(workDir, "custom-tidb-audit.log"), globalLogManager.getLogPath())
	...
	require.Equal(t, "custom-tidb-audit-%e.log", globalLogManager.GetLogConfigPath())
	require.Equal(t, filepath.Join(workDir, "custom-tidb-audit-127-0-0-1-4000.log"), globalLogManager.getLogPath())
	...
	require.Equal(t, tempDirLog, globalLogManager.GetLogConfigPath())
	require.Equal(t, tempDirLog, globalLogManager.getLogPath())
}
...
func TestAuditLogDefaultInstanceLog(t *testing.T) {
	...
	require.Equal(t, "tidb-audit-%e.log", globalLogManager.GetLogConfigPath())
	require.Equal(t, filepath.Join(tempDir, "tidb-audit-127-0-0-1-4000.log"), globalLogManager.getLogPath())
}
...
func TestAuditLogMaxSize(t *testing.T) {
	...
	require.Equal(t, int64(102400), globalLogManager.GetFileMaxSize())
...
func TestAuditLogMaxLifetime(t *testing.T) {
	...
	require.Equal(t, int64(0), globalLogManager.GetFileMaxLifetime())
...
func TestAuditLogReservedBackups(t *testing.T) {
	...
	require.Equal(t, 2, globalLogManager.GetFileReservedBackups())
	tk.MustExec("SET global tidb_audit_log_max_lifetime = 3")
	tk.MustQuery("SELECT @@global.tidb_audit_log_max_lifetime").Check(testkit.Rows("3"))
	...
	require.Equal(t, 0, globalLogManager.GetFileReservedBackups())
	tk.MustExec("SET global tidb_audit_log_max_lifetime = 3")
	tk.MustQuery("SELECT @@global.tidb_audit_log_max_lifetime").Check(testkit.Rows("3"))
	time.Sleep(16 * time.Second)
	tk.MustExec("SET global tidb_audit_log_max_lifetime = 0")
...
// 	files, err := getAllAuditLogs(workDir, "tidb-audit-", ".log")
// 	require.NoError(t, err)
// 	require.Equal(t, 1, len(files), files)
...
// 	require.Equal(t, 1, globalLogManager.GetFileReservedDays())
